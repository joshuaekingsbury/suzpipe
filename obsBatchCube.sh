...
if [[ $_shell == "zsh" ]]; then
    ...
    for line in "${(f)"$(<$obsListFile)"}"
    {
        ...
        osascript -e "on run argv
            tell application \"Terminal\"
                ...
                tell application \"Terminal\" to do script \"ciao\" in selected tab of the front window
                repeat
                    delay 0.1
                    if not busy of window 1 then exit repeat
                end repeat

                # tell application \"Terminal\" to do script \". obsCube.sh \" & item 1 of argv & \";\" in selected tab of the front window
                tell application \"Terminal\" to do script \". obsCube.sh \" & item 1 of argv in selected tab of the front window
    ...
    while read -r line
    do
      gnome-terminal -- bash -c '. obsCube.sh '$line'; exec bash'
    done < "$obsListFile"
